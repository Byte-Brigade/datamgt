...
class DivisionSummarySheet implements FromCollection, ShouldAutoSize, WithTitle, WithHeadings
{
    use Exportable;
    ...
    {
        $number = 1;
        $this->data = $data->groupBy('divisi_pembebanan')->map(function ($perdins, $divisi) {
            $spender = $perdins->flatMap(function ($spender) {
                return $spender->gap_perdin_details;
            });
            return [
                'Divisi Pembebanan' => $divisi,
    ...
    {
        return 'Summary Division - ' . $this->periode;
    }
}
